...
public class JobActionController {
    ...
    @ResponseBody
    public ResponseEntity<Map<String, String>> scoreJobAjax(@PathVariable UUID id) {
        try {
            ollamaMatchmakerService.scoreJob(id);
            return ResponseEntity.ok(Map.of("status", "success", "message", "Matchmaking complete"));
        } catch (Exception e) {
            log.error("Failed to start AI Matchmaking for job {}: {}", id, e.getMessage());
            return ResponseEntity.internalServerError().body(Map.of("status", "error", "message", getRootCause(e).getMessage()));
        }
    }
